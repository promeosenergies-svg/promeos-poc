...
def _collect_frontend_files():
    ...
    return [
        f
        for f in files
        if "__tests__" not in f.parts and not f.name.endswith(".test.js") and not f.name.endswith(".test.jsx")
    ]
...
def test_whitelist_pipeline_enums_allowed():
    ...
    assert len(allowed_display_fields) >= 15, "Whitelist doit contenir ≥ 15 champs display autorisés"
