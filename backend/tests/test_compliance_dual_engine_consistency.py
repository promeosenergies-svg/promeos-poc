...
class TestRuleMapping:
    ...
    def test_regulation_worst_status_aggregation(self):
        pairs = [
            ("DT_OPERAT", "NOK"),
            ("DT_ENERGY_DATA", "UNKNOWN"),
            ("DT_SCOPE", "OK"),
            ("BACS_ATTESTATION", "NOK"),
            ("APER_PARKING", "OK"),
        ]
        worst = regulation_worst_status(pairs)
...
class TestDualEngineConsistency:
    ...
    def _run_compliance_rules(self, db_session, site_id):
        """Invoke compliance_rules.evaluate_site et retourne (rule_id, status)."""
    ...
    def _run_regops(self, db_session, site_id):
        """Invoke regops.engine.evaluate_site et retourne (rule_id, status)."""
        from regops.engine import evaluate_site as eval_regops

        summary = eval_regops(db_session, site_id)
        return [(f.rule_id, f.status) for f in summary.findings]
    ...
    def test_both_engines_flag_issues_on_empty_golden_site(self, db_session, golden_site):
        ...
        """
        try:
            rules_pairs = self._run_compliance_rules(db_session, golden_site.id)
        except Exception as exc:
            pytest.skip(f"compliance_rules.evaluate_site skipped: {exc}")

        try:
            regops_pairs = self._run_regops(db_session, golden_site.id)
        except Exception as exc:
            pytest.skip(f"regops.engine.evaluate_site skipped: {exc}")

        rules_worst = regulation_worst_status(rules_pairs)
        regops_worst = regulation_worst_status(regops_pairs)
        ...
        for reg in CANONICAL_REGULATIONS:
            ...
            if r == STATUS_OK and g == STATUS_ISSUE:
                pytest.fail(
                    ...
                    f"rules_pairs={rules_pairs}, regops_pairs={regops_pairs}"
                )
    ...
    def test_all_emitted_rule_ids_are_mapped(self, db_session, golden_site):
        """Tout rule_id émis par l'un ou l'autre moteur doit figurer dans le
        mapping canonique, sinon RULE_ID_TO_REGULATION doit être mis à jour.
        """
        ...
        all_rule_ids = {rid for rid, _ in rules_pairs} | {rid for rid, _ in regops_pairs}
        unmapped = all_rule_ids - set(RULE_ID_TO_REGULATION.keys())
        assert not unmapped, (
            f"Rule IDs non mappés dans compliance_rule_mapping.RULE_ID_TO_REGULATION : "
            f"{sorted(unmapped)}. Ajouter au mapping pour préserver la cohérence cross-moteur."
        )
