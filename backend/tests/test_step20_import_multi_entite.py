...
class TestTemplateColumns:
    ...
    def test_new_columns_not_required(self):
        from services.import_mapping import CANONICAL_COLUMNS
        new_keys = {"siren_entite", "nom_entite", "portefeuille",
                     "batiment_nom", "batiment_surface_m2",
                     "batiment_annee_construction", "batiment_cvc_power_kw"}
...
class TestBackwardCompat:
    ...
    def test_old_columns_still_present(self):
        from services.import_mapping import CANONICAL_COLUMNS
        keys = [c["key"] for c in CANONICAL_COLUMNS]
        for old_key in ["nom", "adresse", "code_postal", "ville", "surface_m2",
                         "type", "naf_code", "siren", "siret", "energy_type",
                         "delivery_code", "numero_serie", "type_compteur", "puissance_kw"]:
