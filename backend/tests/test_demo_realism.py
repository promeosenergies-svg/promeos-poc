...
class TestWeatherRealism:
    def test_nice_warmer_than_paris_in_winter(self, seeded_db):
        ...
        paris_jan = db.query(func.avg(EmsWeatherCache.temp_avg_c)).filter(
            EmsWeatherCache.site_id == paris.id,
            func.strftime("%m", EmsWeatherCache.date) == "01"
        ).scalar()
        nice_jan = db.query(func.avg(EmsWeatherCache.temp_avg_c)).filter(
            EmsWeatherCache.site_id == nice.id,
            func.strftime("%m", EmsWeatherCache.date) == "01"
        ).scalar()
    ...
    def test_weather_min_less_than_avg_less_than_max(self, seeded_db):
        ...
        db, _ = seeded_db
        violations = db.query(EmsWeatherCache).filter(
    ...
    def test_ar1_autocorrelation(self, seeded_db):
        ...
        changes = [abs(records[i].temp_avg_c - records[i - 1].temp_avg_c)
...
class TestConsumptionRealism:
    ...
    def test_hotel_has_higher_weekend_than_office(self, seeded_db):
        ...
        assert nice
        hotel_meter = db.query(Meter).filter_by(
            site_id=nice.id, energy_vector=EnergyVector.ELECTRICITY
        ).first()
        assert hotel_meter

        weekend_count = db.query(MeterReading).filter(
    ...
    def test_surface_normalized_in_range(self, seeded_db):
        ...
        for site in sites:
            meter = db.query(Meter).filter_by(
                site_id=site.id, energy_vector=EnergyVector.ELECTRICITY
            ).first()
            if not meter or not site.surface_m2:
                continue

            total_kwh = db.query(func.sum(MeterReading.value_kwh)).filter(
            ...
            assert 20 < kwh_per_m2 < 600, \
                f"{site.nom}: {kwh_per_m2:.0f} kWh/m²/an out of range"
...
class TestGasReadings:
    ...
    def test_gas_higher_in_winter(self, seeded_db):
        ...
        winter_avg = db.query(func.avg(MeterReading.value_kwh)).filter(
            MeterReading.meter_id == gas_meter.id,
            func.strftime("%m", MeterReading.timestamp).in_(["01", "02", "12"])
        ).scalar()
        summer_avg = db.query(func.avg(MeterReading.value_kwh)).filter(
            MeterReading.meter_id == gas_meter.id,
        ...
        if winter_avg and summer_avg and summer_avg > 0:
            assert winter_avg > summer_avg, \
                f"Gas winter ({winter_avg:.1f}) should be > summer ({summer_avg:.1f})"
...
class Test15MinRealism:
    def test_365_days_of_15min_data(self, seeded_db):
        ...
        count = db.query(MeterReading).filter_by(
            meter_id=elec_meter.id, frequency=FrequencyType.MIN_15
        ).count()
    ...
    def test_15min_sum_approx_hourly(self, seeded_db):
        ...
        sample = db.query(MeterReading).filter_by(
            meter_id=elec_meter.id, frequency=FrequencyType.MIN_15
        ).first()
        if not sample:
        ...
        slots = db.query(MeterReading).filter(
            MeterReading.meter_id == elec_meter.id,
            MeterReading.frequency == FrequencyType.MIN_15,
            MeterReading.timestamp >= hour_ts,
            MeterReading.timestamp < hour_ts.replace(hour=hour_ts.hour + 1) if hour_ts.hour < 23
...
class TestAnomalies:
    def test_quality_score_below_1_exists(self, seeded_db):
        ...
        db, _ = seeded_db
        anomaly_count = db.query(MeterReading).filter(
    ...
    def test_multiple_anomaly_types(self, seeded_db):
        ...
        assert sites_with_anomalies >= 2, \
            f"Only {sites_with_anomalies} sites have anomalies"
...
class TestUsageBreakdown:
    ...
    def test_usage_has_cvc(self, seeded_db):
        ...
        batiments = db.query(Batiment).all()
        for bat in batiments:
            cvc = db.query(Usage).filter_by(
                batiment_id=bat.id, type=TypeUsage.CVC
            ).first()
...
class TestMonitoringDepth:
    ...
    def test_alerts_have_start_ts(self, seeded_db):
        ...
        db, _ = seeded_db
        with_ts = db.query(MonitoringAlert).filter(
            MonitoringAlert.start_ts.isnot(None)
        ).count()
