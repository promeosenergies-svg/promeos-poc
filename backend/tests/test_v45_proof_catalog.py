...
class TestProofsStatusEndpoint:
    ...
    def test_proofs_status_shape(self):
        ...
        assert resp.status_code == 200
        data = resp.json()
        for key in ["expected", "expected_count", "deposited", "deposited_count",
                     "validated", "validated_count", "missing", "missing_count", "coverage_pct"]:
    ...
    def test_proofs_baseline_has_expected(self):
        ...
        client = TestClient(app)
        efa = client.post("/api/tertiaire/efa", json={
            "org_id": 1, "nom": "V45 Expected Test",
        })
        if efa.status_code != 201:
            pytest.skip("Cannot create EFA")
        efa_id = efa.json()["id"]
...
class TestProofLinkEndpoint:
    def test_link_proof_creates_artifact(self):
        ...
        client = TestClient(app)
        efa = client.post("/api/tertiaire/efa", json={
            "org_id": 1, "nom": "V45 Link Test",
        })
        if efa.status_code != 201:
            pytest.skip("Cannot create EFA")
        efa_id = efa.json()["id"]
        resp = client.post(f"/api/tertiaire/efa/{efa_id}/proofs/link", json={
...
class TestV45SourceGuards:
    ...
    def _load(self):
        self.proofs_code = (
            Path(__file__).resolve().parent.parent / "services" / "tertiaire_proofs.py"
