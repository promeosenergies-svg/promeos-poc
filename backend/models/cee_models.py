...
class WorkPackage(Base, TimestampMixin):
    ...
    size = Column(
        SAEnum(WorkPackageSize), nullable=False, default=WorkPackageSize.M,
    ...
    cee_status = Column(
        SAEnum(CeeStatus), nullable=False, default=CeeStatus.A_QUALIFIER,
...
class CeeDossier(Base, TimestampMixin):
    ...
    work_package_id = Column(
        Integer, ForeignKey("work_packages.id"),
    ...
    current_step = Column(
        SAEnum(CeeDossierStep), nullable=False, default=CeeDossierStep.DEVIS,
...
class CeeDossierEvidence(Base, TimestampMixin):
    ...
    id = Column(Integer, primary_key=True, index=True)
    dossier_id = Column(
        Integer, ForeignKey("cee_dossiers.id"),
        nullable=False, index=True,
    )
    site_id = Column(
        Integer, ForeignKey("sites.id"),
    ...
    statut = Column(
        SAEnum(StatutEvidence), nullable=False, default=StatutEvidence.MANQUANT,
    )
    ...
    evidence_id = Column(
        Integer, ForeignKey("evidences.id"),
        nullable=True,
