...
class PurchaseAssumptionSet(Base, TimestampMixin):
    ...
    energy_type = Column(
        Enum(BillingEnergyType), nullable=False, default=BillingEnergyType.ELEC,
    ...
    scenario_results = relationship(
        "PurchaseScenarioResult", back_populates="assumption_set",
...
class PurchaseScenarioResult(Base, TimestampMixin):
    ...
    assumption_set_id = Column(
        Integer, ForeignKey("purchase_assumption_sets.id"), nullable=False, index=True,
        comment="Jeu d'hypotheses parent",
    )
    strategy = Column(
        Enum(PurchaseStrategy), nullable=False,
    ...
    risk_score = Column(
        Float, nullable=False, default=50,
    ...
    reco_status = Column(
        Enum(PurchaseRecoStatus), default=PurchaseRecoStatus.DRAFT, nullable=False,
