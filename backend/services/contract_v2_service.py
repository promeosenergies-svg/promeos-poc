...
def resolve_pricing(db: Session, annexe: ContractAnnexe) -> List[Dict[str, Any]]:
    """Retourne pricing effectif: override si has_price_override, sinon cadre.
    Chaque ligne porte 'source': 'cadre' | 'override'."""
    ...
    # Fallback: colonnes plates sur EnergyContract
    if cadre:
        lines = []
...
def coherence_check(db: Session, cadre_id: int) -> List[Dict[str, str]]:
    ...
    # R1: Cadre sans annexe
    if not annexes:
        results.append({"rule_id": "R1", "level": "warning", "message": "Cadre sans annexe site"})

    # R2: Annexe sans PDL/PRM
    for a in annexes:
        if not a.delivery_point_id:
            results.append(
    ...
    # R4: HP/HC sans heures creuses dans TOUSchedule (simplified check)
    for a in annexes:
        if a.tariff_option and a.tariff_option.value in ("hp_hc", "cu4", "mu4"):
            pass  # Full TOUSchedule check would go here

    # R5: Prix indexe sans source d'indice
            ...
            results.append(
                {
                    "rule_id": "R6",
                    "level": "warning",
                    "message": f"Puissance non renseignee pour {a.annexe_ref or a.id}",
                }
            )

    # R7: Tolerance sans engagement volume
    for a in annexes:
            ...
            results.append(
                {
                    "rule_id": "R7",
                    "level": "warning",
                    "message": f"Penalite sans volume engage pour {a.annexe_ref or a.id}",
                }
            )

    # R8: Dates incoherentes (fin < debut)
    ...
    # R10: Contrat expire avec factures non couvertes (simplified)
    if contract.contract_status and contract.contract_status.value == "expired":
        results.append(
            {"rule_id": "R10", "level": "warning", "message": "Contrat expire — verifier couverture factures"}
        )

    # R11: Annexe override sans pricing
    for a in annexes:
            ...
            results.append(
                {
                    "rule_id": "R11",
                    "level": "error",
                    "message": f"Annexe {a.annexe_ref or a.id} override active sans grille prix",
                }
            )

    # R12: Indexation ARENH sur contrat post-2025
    if contract.offer_indexation and "arenh" in (contract.offer_indexation.value or "").lower():
        if contract.start_date and contract.start_date.year > 2025:
            results.append(
                {
                    "rule_id": "R12",
                    "level": "info",
                    "message": "ARENH supprime fin 2025 — verifier indexation post-ARENH/VNU",
                }
            )
