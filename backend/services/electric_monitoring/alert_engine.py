...
class AlertEngine:
    ...
    def evaluate(self, kpis: Dict[str, Any],
                 power_risk: Dict[str, Any],
                 data_quality: Dict[str, Any],
                 previous_kpis: Optional[Dict[str, Any]] = None,
                 site_id: Optional[int] = None,
                 meter_id: Optional[int] = None) -> List[Dict[str, Any]]:
        """
        ...
        if we_ratio > thr_we:
            alerts.append(self._make_alert(
                "WEEKEND_ANORMAL", site_id, meter_id, now,
        ...
        if previous_kpis:
            ...
            if prev_pbase > 0:
                ...
                if pbase_change_pct > thr_derive:
                    alerts.append(self._make_alert(
                        "DERIVE_TALON", site_id, meter_id, now,
                        explanation=f"Base load increased by {pbase_change_pct:.1f}% "
                                    f"({prev_pbase:.1f} -> {curr_pbase:.1f} kW).",
                        evidence={"previous_pbase_kw": prev_pbase, "current_pbase_kw": curr_pbase,
        ...
        if peak_to_avg > thr_peak:
            alerts.append(self._make_alert(
                "PIC_ANORMAL", site_id, meter_id, now,
                severity="high",
                ...
                evidence={"peak_to_average": peak_to_avg, "threshold": thr_peak,
                          "pmax_kw": kpis.get("pmax_kw", 0), "pmean_kw": kpis.get("pmean_kw", 0)},
        ...
        if previous_kpis:
            ...
            if prev_p95 > 0:
                ...
                if p95_change_pct > thr_p95:
                    alerts.append(self._make_alert(
                        "P95_HAUSSE", site_id, meter_id, now,
                        explanation=f"P95 increased by {p95_change_pct:.1f}% "
                                    f"({prev_p95:.1f} -> {curr_p95:.1f} kW).",
                        evidence={"previous_p95_kw": prev_p95, "current_p95_kw": curr_p95,
        ...
        if total_profile_kwh > 0:
            ...
            if off_hours_pct > thr_off:
                alerts.append(self._make_alert(
                    "HORS_HORAIRES", site_id, meter_id, now,
                    severity="info",
        ...
        if load_factor > thr_lf:
            alerts.append(self._make_alert(
                "COURBE_PLATE", site_id, meter_id, now,
                severity="info",
        ...
        if completeness < thr_comp:
            alerts.append(self._make_alert(
                "DONNEES_MANQUANTES", site_id, meter_id, now,
                severity="high",
        ...
        if dup_count > 0 or dst_count > 0:
            alerts.append(self._make_alert(
                "DOUBLONS_DST", site_id, meter_id, now,
        ...
        if neg_count > 0:
            alerts.append(self._make_alert(
                "VALEURS_NEGATIVES", site_id, meter_id, now,
                severity="critical",
    ...
    def _make_alert(self, alert_type: str,
                    ...
                    estimated_impact_eur: float = 0) -> Dict[str, Any]:
