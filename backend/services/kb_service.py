...
class KBService:
    ...
    def load_kb_version_from_manifest(self, manifest_path: Path) -> Optional[KBVersion]:
        ...
        existing = self.db.query(KBVersion).filter_by(
        ...
        for yaml_file in yaml_files:
            try:
                ...
                if reco_data['impact_score'] and reco_data['confidence_score'] and reco_data['ease_score']:
                    reco_data['ice_score'] = (
                        reco_data['impact_score'] *
                        reco_data['confidence_score'] *
                        reco_data['ease_score']
                    ) / 1000.0
    ...
    def get_archetype_by_code(self, code: str) -> Optional[KBArchetype]:
        ...
        return self.db.query(KBArchetype).filter_by(
            code=code,
            status=KBStatus.VALIDATED
        ).first()
    ...
    def get_archetype_by_naf(self, naf_code: str) -> Optional[KBArchetype]:
        ...
        mapping = self.db.query(KBMappingCode).filter_by(
            naf_code=naf_code
        ).order_by(KBMappingCode.priority.desc()).first()
    ...
    def search_archetypes(self, segment: Optional[str] = None, kwh_range: Optional[tuple] = None) -> List[KBArchetype]:
        ...
        if kwh_range:
            ...
            query = query.filter(
                and_(
                    KBArchetype.kwh_m2_min <= kwh_max,
                    KBArchetype.kwh_m2_max >= kwh_min
                )
            )
    ...
    def get_recommendations(self, archetype_code: Optional[str] = None, anomaly_code: Optional[str] = None) -> List[KBRecommendation]:
