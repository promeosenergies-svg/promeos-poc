        ...
        for month_offset in range(_NUM_MONTHLY_SNAPSHOTS):
            ...
            if month_offset == 0:
                site_alerts = _generate_alerts(
                    site, meter, kpis, power_risk, quality, snapshot.id, rng,
                )
...
def _generate_insights(site, meter, kpis, rng: random.Random,
                       month_offset: int = 0) -> list:
    ...
    if we_ratio > 0.7 and kpis.get("total_kwh", 0) > 1000:
        insights.append(ConsumptionInsight(
            site_id=site.id, meter_id=meter.id,
            type="weekend_ratio", severity="medium",
