        ...
        if cvc_kw > 290:
            ...
            findings.append(
                ComplianceFinding(
                    site_id=site.id,
                    regulation="bacs",
                    rule_id="BACS_LOW_DEADLINE",
                    status="UNKNOWN" if is_unknown else "OK",
                    severity="high" if is_unknown else "low",
                    deadline=date(2030, 1, 1),
                    evidence="CVC 70-290 kW" + (" — donnees CVC incompletes" if is_unknown else " — conforme"),
                    run_batch_id=batch.id,
            ...
            if bucket == 0:
                ...
                findings.append(
                    ComplianceFinding(
                        site_id=site.id,
                        regulation="decret_tertiaire_operat",
                        rule_id="DT_ENERGY_DATA",
                        status="UNKNOWN",
                        severity="medium",
                        deadline=date(2030, 12, 31),
                        evidence="Donnees de consommation partielles, declaration incomplete",
                        recommended_actions_json=json.dumps(
                            ["Completer les donnees energetiques annuelles", "Soumettre la declaration sur OPERAT"]
                        ),
                        run_batch_id=batch.id,
        ...
        if parking >= 1500:
            ...
            findings.append(
                ComplianceFinding(
                    site_id=site.id,
                    regulation="aper",
                    rule_id="APER_TOITURE",
                    status="UNKNOWN",
                    severity="medium",
                    evidence=f"Toiture {roof:.0f} m2, potentiel ENR a evaluer",
                    recommended_actions_json=json.dumps(["Evaluer le potentiel solaire de la toiture"]),
                    run_batch_id=batch.id,
                    insight_status=InsightStatus.OPEN,
                )
            )
