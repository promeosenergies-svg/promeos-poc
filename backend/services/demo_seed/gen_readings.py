...
_PROFILE_TO_TYPE = {
    "office": "bureau", "hotel": "hotel", "warehouse": "entrepot",
    "school": "enseignement", "hospital": "sante", "retail": "commerce",
}
...
_HOURLY_PATTERN = {
    "office": [
        0.12, 0.10, 0.10, 0.10, 0.11, 0.14,
        0.22, 0.48, 0.74, 0.92, 1.00, 0.96,
        0.76, 0.79, 1.00, 0.97, 0.92, 0.74,
        0.50, 0.30, 0.22, 0.17, 0.14, 0.12,
    ],
    "hotel": [
        0.52, 0.48, 0.44, 0.42, 0.44, 0.55,
        0.68, 0.88, 0.96, 0.90, 0.82, 0.88,
        0.90, 0.84, 0.78, 0.80, 0.85, 0.92,
        1.00, 1.00, 0.96, 0.88, 0.74, 0.62,
    ],
    "warehouse": [
        0.28, 0.25, 0.22, 0.22, 0.26, 0.42,
        0.65, 0.85, 1.00, 1.00, 0.97, 0.82,
        0.58, 0.80, 0.98, 1.00, 0.95, 0.80,
        0.55, 0.40, 0.33, 0.30, 0.28, 0.28,
    ],
    "school": [
        0.10, 0.08, 0.08, 0.08, 0.09, 0.14,
        0.25, 0.52, 0.82, 0.94, 1.00, 0.96,
        0.62, 0.52, 0.88, 0.94, 0.75, 0.50,
        0.26, 0.18, 0.14, 0.12, 0.10, 0.10,
    ],
    "hospital": [
        0.65, 0.60, 0.56, 0.55, 0.58, 0.65,
        0.75, 0.86, 0.94, 0.99, 1.00, 0.99,
        0.94, 0.92, 0.98, 1.00, 0.99, 0.95,
        0.90, 0.85, 0.82, 0.78, 0.74, 0.68,
    ],
    "retail": [
        0.12, 0.10, 0.08, 0.08, 0.10, 0.16,
        0.22, 0.40, 0.65, 0.84, 0.95, 1.00,
        1.00, 0.98, 0.96, 0.99, 1.00, 0.95,
        0.85, 0.72, 0.52, 0.30, 0.18, 0.13,
    ],
}
...
_HOTEL_OCCUPANCY = {
    ...
    "Nice":      {1: 0.45, 2: 0.50, 3: 0.55, 4: 0.65, 5: 0.75, 6: 0.90,
                  7: 0.95, 8: 0.95, 9: 0.85, 10: 0.65, 11: 0.50, 12: 0.45},
    "Paris":     {1: 0.65, 2: 0.68, 3: 0.75, 4: 0.82, 5: 0.85, 6: 0.80,
                  7: 0.75, 8: 0.70, 9: 0.82, 10: 0.80, 11: 0.72, 12: 0.68},
    "Marseille": {1: 0.40, 2: 0.45, 3: 0.55, 4: 0.65, 5: 0.75, 6: 0.88,
                  7: 0.92, 8: 0.92, 9: 0.80, 10: 0.60, 11: 0.45, 12: 0.42},
    "Lyon":      {1: 0.55, 2: 0.58, 3: 0.65, 4: 0.70, 5: 0.72, 6: 0.75,
                  7: 0.68, 8: 0.62, 9: 0.72, 10: 0.70, 11: 0.60, 12: 0.55},
    "Toulouse":  {1: 0.50, 2: 0.55, 3: 0.60, 4: 0.65, 5: 0.70, 6: 0.75,
                  7: 0.72, 8: 0.68, 9: 0.70, 10: 0.65, 11: 0.55, 12: 0.52},
}
...
_LEGACY_BASE_KW = {
    "office": 80, "hotel": 65, "warehouse": 110,
    "school": 70, "hospital": 85, "retail": 75,
}
...
def _build_anomaly_schedule(site_idx: int, profile_name: str, days: int,
    ...
    if profile_name == "office" and site_idx == 0:
        ...
        for d in range(drift_start, min(drift_start + 15, days)):
            ...
            anomalies.append({
                "day": d, "hours": list(range(0, 6)),
                "type": "cvc_drift", "multiplier": mult, "quality": 0.75,
            })
        ...
        for d in heat_days:
            anomalies.append({
                "day": d, "hours": list(range(12, 20)),
                "type": "heatwave_spike", "multiplier": 1.40, "quality": 0.65,
            })
        ...
        for d in range(outage_start, min(outage_start + 7, days)):
            anomalies.append({
                "day": d, "hours": list(range(24)),
                "type": "partial_outage", "multiplier": 0.50, "quality": 0.60,
            })
    ...
    for target_month in [4, 10]:
        ...
        for d in range(days):
            ...
            if dt.month == target_month and 10 <= dt.day <= 12:
                anomalies.append({
                    "day": d, "hours": list(range(8, 18)),
                    "type": "season_transition", "multiplier": 1.10, "quality": 0.85,
                })
...
def generate_readings(db, meters: list, site_profiles: dict,
                      temp_lookup: dict, days: int, rng: random.Random,
                      site_meta: dict = None) -> int:
    ...
    for meter_idx, meter in enumerate(meters):
        ...
        readings = _gen_meter_readings(
            meter.id, profile_name, profile, site_temps, start, days,
            base_kw, psub, rng, vacation_weeks, city, anomaly_lookup
        )
...
def _gen_meter_readings(meter_id: int, profile_name: str, profile: dict,
                        ...
                        base_kw: float, psub: float,
                        rng: random.Random, vacation_weeks: list,
                        city: str, anomaly_lookup: dict) -> list:
...
def generate_gas_readings(db, meters: list, site_profiles: dict,
                          temp_lookup: dict, days: int, rng: random.Random,
                          site_meta: dict = None) -> int:
...
_MONTHLY_BASE = {
    "office": 25000, "hotel": 80000, "retail": 35000,
    "warehouse": 50000, "school": 18000, "hospital": 120000,
}


def generate_monthly_readings(db, meters: list, site_profiles: dict,
                              months: int, rng: random.Random,
                              site_meta: dict = None) -> int:
...
def generate_15min_readings(db, meters: list, site_profiles: dict,
    ...
    for meter_idx, meter in enumerate(meters):
        ...
        readings = _gen_15min_meter_readings(
            meter.id, profile_name, profile, site_temps, start, days,
            base_kw, psub, max_kw, rng, vacation_weeks, city, anomaly_lookup
        )
        _bulk_insert_ignore(db, readings)
        total += len(readings)
...
def _gen_15min_meter_readings(meter_id: int, profile_name: str, profile: dict,
                               temps: dict, start: datetime, days: int,
                               base_kw: float, psub: float, max_kw: float,
                               rng: random.Random, vacation_weeks: list,
                               city: str, anomaly_lookup: dict) -> list:
