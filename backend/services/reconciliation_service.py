...
def reconcile_site(db: Session, site_id: int) -> dict:
    ...
    active_contracts = [
        c
        for c in contracts
        if (c.start_date is None or c.start_date <= today) and (c.end_date is None or c.end_date >= today)
    ]
    invoices = db.query(EnergyInvoice).filter(EnergyInvoice.site_id == site_id).all()
    recent_invoices = [inv for inv in invoices if inv.issue_date and inv.issue_date >= six_months_ago]
    ...
    if active_contracts and recent_invoices:
        earliest_contract_start = min((c.start_date for c in active_contracts if c.start_date), default=None)
...
def fix_create_delivery_point(
    db: Session, site_id: int, code: str, energy_type: str = "ELEC", applied_by: str = None
) -> dict:
    ...
    _log_fix(
        ...
        "fail",
        "ok",
        {"delivery_point_id": dp.id, "code": code},
        applied_by,
    )
    return {"delivery_point_id": dp.id, "code": code}


def fix_extend_contract(db: Session, site_id: int, contract_id: int, months: int = 12, applied_by: str = None) -> dict:
    ...
    old_end = ct.end_date
    today = date.today()
    new_end = date(today.year + (today.month + months - 1) // 12, (today.month + months - 1) % 12 + 1, 1)
    ct.end_date = new_end
    db.flush()
    _log_fix(
        db,
...
def fix_adjust_contract_dates(
    db: Session, site_id: int, contract_id: int, new_start: str = None, applied_by: str = None
) -> dict:
...
def fix_create_payment_rule(
    db: Session,
    site_id: int,
    invoice_entity_id: int,
    payer_entity_id: int = None,
