...
export default function ExportPackRFP({ portfolio, sites, orgName, onClose }) {
  ...
  const handlePrint = () => {
    ...
    doc.open();
    doc.write(
      '<!DOCTYPE html><html><head><title>Pack RFP</title><style>' +
        styles +
  ...
  const totalVolume =
    portfolio.total_volume_kwh_an || sites?.reduce((s, si) => s + (si.volume_kwh_an || 0), 0) || 0;
        ...
        <div ref={printRef} className="p-8">
          ...
          <div className="section">
            ...
            <div className="kpi-strip">
              ...
              <div className="kpi">
                <div className="kpi-val" style={{ color: '#2563eb' }}>
                  {Math.round(portfolio.total_annual_cost_eur || 0).toLocaleString()}
                </div>
              ...
              <div className="kpi">
                <div
                  className="kpi-val"
                  style={{ color: portfolio.weighted_savings_pct > 0 ? '#16a34a' : '#dc2626' }}
                >
          ...
          <div className="summary-box">
            <p>
              ...
              <strong>{Math.round(totalVolume).toLocaleString()} kWh/an</strong>. Le risque moyen
                  ...
                  return (
                    <tr key={site.site_id} className="reco">
                      ...
                      <td style={{ textAlign: 'right' }}>
                        {Math.round(site.volume_kwh_an || 0).toLocaleString()}
                      </td>
                      ...
                      <td
                        style={{
                          textAlign: 'right',
                          color: reco.savings_vs_current_pct > 0 ? '#16a34a' : '#dc2626',
                        }}
                      >
                      ...
                      <td style={{ textAlign: 'center' }}>
                        <span className={`risk-badge ${rb.cls}`}>
                          {rb.label} ({reco.risk_score})
                        </span>
                      </td>
                    </tr>
                  );
                  ...
                  {(site.scenarios || []).map((s) => {
                    ...
                    return (
                      <tr key={s.strategy || s.id} className={s.is_recommended ? 'reco' : ''}>
                        ...
                        <td style={{ textAlign: 'right' }}>
                          {Math.round(s.total_annual_eur).toLocaleString()} EUR
                        </td>
                        <td
                          style={{
                            textAlign: 'right',
