/**
 * PROMEOS — PurchaseAssistantWizard
 * 8-step wizard extracted from PurchaseAssistantPage for embedding as tab.
 * Props: scopedSites, orgId, expertMode, onComplete, initialStep, initialSiteId, initialOffer
 */
...
// ── Carte site réutilisable (demo + API) ──────────────────────────

function SiteCard({ site, selected, onToggle }) {
  return (
    <button
...
// ── Helper téléchargement fichier ─────────────────────────────────

function downloadBlob(content, type, filename, toastFn, toastMsg) {
...
// ── Axis label mapping (scoring) ──────────────────────────────────

const AXIS_LABELS = {
  budgetRisk: 'Budget',
  transparency: 'Transp.',
  contractRisk: 'Contrat',
  dataReadiness: 'Data',
};
...
export default function PurchaseAssistantWizard({
  ...
  useEffect(() => {
    let cancelled = false;
    getPurchaseAssistantData(orgId ?? scope?.orgId ?? null)
      .then((data) => {
        if (!cancelled) {
  ...
  // Deep-link — jump to step on mount
  useEffect(() => {
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 3:
        return <StepHorizon wizard={wizard} setWizard={setWizard} isExpert={expertMode} />;
      case 4:
        return (
          <StepOffers
            wizard={wizard}
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between pt-4 border-t border-gray-200">
        ...
        {step < STEPS.length - 1 ? (
          ...
          <button
            onClick={onComplete}
            className="text-sm text-green-600 font-medium flex items-center gap-1 hover:text-green-700 transition"
          >
    ...
    <div className="space-y-4">
      ...
      {isDemo ? (
                ...
                {org.entities
                  ...
                  .map((site) => (
                    <SiteCard
                      key={site.id}
                      site={site}
                      selected={wizard.selectedSiteIds.includes(site.id)}
                      onToggle={toggleSite}
                    />
                  ))}
              </div>
        ...
        <div className="space-y-3">
          <div className="bg-green-50 border border-green-200 rounded-lg px-4 py-2 text-sm text-green-700">
            {demoSites.length} {demoSites.length > 1 ? 'sites chargés' : 'site chargé'} depuis votre
            patrimoine.
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {demoSites.map((site) => (
      ...
      {wizard.selectedSiteIds.length > 0 && (
        <div className="bg-blue-50 rounded-lg p-3 text-sm text-blue-700">
          <strong>{wizard.selectedSiteIds.length}</strong>{' '}
          {wizard.selectedSiteIds.length > 1 ? 'sites sélectionnés' : 'site sélectionné'}
        </div>
      )}
...
function StepConsumption({ wizard, setWizard, sitesData, isDemo, expertMode }) {
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <KpiCard
          ...
          sublabel={
            expertMode
              ? `Source: ${sitesData.consumption?.source || (isDemo ? 'demo' : 'patrimoine')}`
              : `Basé sur vos relevés (12 mois)`
          }
        ...
        <KpiCard
          ...
          sublabel={`${sitesData.anomalies?.length || 0} anomalie${(sitesData.anomalies?.length || 0) > 1 ? 's' : ''} B2`}
        ...
        <Card>
          ...
          <CardBody>
            ...
            <p className="text-xs text-gray-400 mt-2">
              Saisonnalité mensuelle (coefficient normalisé)
            </p>
          </CardBody>
        </Card>
      )}
      ...
      {sitesData.anomalies?.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
          <div className="flex items-center gap-2 text-sm font-medium text-amber-700 mb-2">
            <AlertTriangle size={16} /> {sitesData.anomalies.length} anomalie
            {sitesData.anomalies.length > 1 ? 's' : ''} de facturation détectée
            {sitesData.anomalies.length > 1 ? 's' : ''}
          </div>
          <ul className="space-y-1">
            {sitesData.anomalies.slice(0, 3).map((a, i) => (
              <li key={i} className="text-xs text-amber-600">
                {a.message}{' '}
                {a.estimatedLossEur > 0 && `(~${a.estimatedLossEur.toLocaleString('fr-FR')} € HT)`}
              </li>
            ))}
          </ul>
        </div>
...
function StepPersona({ wizard, setWizard }) {
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold text-gray-800">Profil décideur</h3>
      <p className="text-sm text-gray-500">
              ...
              <div className="grid grid-cols-4 gap-2 text-[10px]">
                {Object.entries(profile.weights).map(([axis, w]) => (
                  <div key={axis} className="text-center">
                    ...
                    <div className="text-gray-400">{AXIS_LABELS[axis] || axis}</div>
                  </div>
                ))}
              </div>
            </button>
      ...
      <Card>
        <CardBody>
          ...
          <p className="text-xs text-gray-400 mt-1">
            Si renseigné, le moteur calculera la probabilité de dépassement.
          </p>
        </CardBody>
      </Card>
    </div>
...
function StepHorizon({ wizard, setWizard, isExpert }) {
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold text-gray-800">Horizon & Paramètres de risque</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card>
          <CardHeader>Scénario de marché</CardHeader>
          <CardBody>
      ...
      {isExpert && (
        <Card>
          <CardHeader>Paramètres avancés (Expert)</CardHeader>
          <CardBody>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">
                  Itérations MC (max 200)
                </label>
                <input
                  type="number"
                  min={10}
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          {isDemo && (
            <button onClick={loadDemoOffers} className="text-xs text-blue-600 hover:underline">
              Charger les {DEMO_OFFERS.length} offres démo
            </button>
          )}
      ...
      {isDemo && offers.length === 0 && wizard.offers.length === 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-700">
          ...
          En mode démo, {DEMO_OFFERS.length} offres sont pré-chargées automatiquement. Vous pouvez
          aussi en créer manuellement.
        </div>
      )}
      ...
      {wizard.offers.length > 0 && (
        <p className="text-xs text-gray-400">
          {wizard.offers.length} offre{wizard.offers.length > 1 ? 's' : ''} saisie
          {wizard.offers.length > 1 ? 's' : ''} manuellement
        </p>
      )}
    </div>
  );
    ...
    >
      ...
      {!readOnly && (
        <div className="px-4 pb-3 flex items-center gap-4">
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                const SHARE_PRESETS = {
                  [OfferStructure.FIXE]: { fixedSharePct: 1, indexedSharePct: 0, spotSharePct: 0 },
                  [OfferStructure.INDEXE]: {
                    fixedSharePct: 0,
                    indexedSharePct: 1,
                    spotSharePct: 0,
                  },
                  [OfferStructure.SPOT]: { fixedSharePct: 0, indexedSharePct: 0, spotSharePct: 1 },
                };
                onUpdate(offer.id, { structure: s });
                if (SHARE_PRESETS[s]) onUpdatePricing(offer.id, SHARE_PRESETS[s]);
              }}
            >
          ...
          {(offer.structure === OfferStructure.FIXE ||
            ...
            <div>
              <label className="text-xs text-gray-500">Prix fixe (€ HT/MWh)</label>
              <input
                type="number"
          ...
          {(offer.structure === OfferStructure.INDEXE ||
            ...
            <>
              <div>
                <label className="text-xs text-gray-500">Spread (€ HT/MWh)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="text-xs text-gray-500">Cap (€ HT/MWh)</label>
                <input
                  type="number"
                  placeholder="—"
                  className="block w-20 border border-gray-300 rounded px-2 py-1 text-sm mt-0.5"
          ...
          {offer.structure === OfferStructure.HYBRIDE && (
            <>
              ...
              <div>
                <label className="text-xs text-gray-500">% Indexé</label>
                <input
                  type="number"
                  min={0}
                  max={100}
      ...
      {expanded && (
        <div className="px-4 pb-4 pt-2 border-t border-gray-100 text-xs text-gray-500 space-y-2">
          <div>
            <strong>Contrat:</strong> {offer.contractTerms?.durationMonths}m, préavis{' '}
            {offer.contractTerms?.noticePeriodDays}j, résiliation:{' '}
          ...
          <div>
            <strong>Intermédiation:</strong>{' '}
            {offer.intermediation?.hasIntermediary
              ? `Oui (${offer.intermediation.feeDisclosed ? offer.intermediation.feeEurPerMwh + ' € HT/MWh' : 'Non divulgué'})`
              : 'Non'}
          </div>
          <div>
          ...
          <div>
            <strong>Décomposition:</strong> {offer.breakdown?.length || 0} composante
            {(offer.breakdown?.length || 0) > 1 ? 's' : ''} (
            {offer.breakdown?.filter((b) => b.status === 'KNOWN').length || 0} connue
            {(offer.breakdown?.filter((b) => b.status === 'KNOWN').length || 0) > 1 ? 's' : ''})
          </div>
        ...
        <KpiCard
          ...
          value={
            scoredOffers.length > 0 && scoredOffers.some((s) => s.corridor?.p50 != null)
              ? `${fmtNum(Math.min(...scoredOffers.filter((s) => s.corridor?.p50 != null).map((s) => s.corridor.p50)), 1)} € HT/MWh`
              : '—'
          }
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-gray-500 uppercase text-xs">
            <tr>
              ...
              <th className="px-4 py-3 text-right">Coût HT/an</th>
              ...
              return (
                <tr key={s.offerId} className={isBest ? 'bg-blue-50' : 'hover:bg-gray-50'}>
                  ...
                  <td className="px-4 py-3 text-center">
                    <span
                      className={`inline-block w-10 text-center font-bold text-xs py-0.5 rounded ${scoreColorClass(s.scores.overall)}`}
                    >
                      {s.scores.overall}
                    </span>
                  </td>
...
function StepScoring({ scoredOffers, recommendation }) {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold text-gray-800">Scoring détaillé & Transparence</h3>
      ...
      {selected && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            { key: 'budgetRisk', label: 'Risque Budgétaire', icon: AlertTriangle },
      ...
      {selected?.offer?.breakdown?.length > 0 && (
        <Card>
          <CardHeader>Décomposition tarifaire — {selected.supplierName}</CardHeader>
          <CardBody>
                ...
                <div key={i} className="flex items-center justify-between text-sm py-1">
                  ...
                  <div className="flex items-center gap-4 text-xs">
                    ...
                    <span className="text-gray-700 font-medium w-20 text-right">
                      {b.eurPerMwh != null ? `${fmtNum(b.eurPerMwh, 2)} € HT/MWh` : 'est.'}
                    </span>
                    <Badge
                      variant={
...
function StepDecision({
  ...
  const resolvedScored = recommendation._scoredOffers || scoredOffers;
  const resolvedResults = engineOutput?.results || [];
  ...
  const handleExportCsv = () => {
    const csv = generateComparisonCsv(resolvedScored, resolvedResults);
    downloadBlob(
      csv,
      'text/csv;charset=utf-8',
      `promeos_b3_comparaison_${dateSlug}.csv`,
      toast,
  ...
  const handleExportDecisionNote = () => {
    const note = generateDecisionNote({
      recommendation,
      scoredOffers: resolvedScored,
      offerResults: resolvedResults,
      offers,
      persona: wizard.persona,
      ...commonExportParams,
    });
    downloadBlob(
      ...
      'application/json',
      `promeos_b3_note_decision_${dateSlug}.json`,
      toast,
      'Note de décision exportée'
    );
  };
  ...
  const handleExportRfp = () => {
    ...
    downloadBlob(
      ...
      'application/json',
      `promeos_b3_pack_rfp_${dateSlug}.json`,
      toast,
      'Pack RFP exporté'
    );
  };
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold text-gray-800">Recommandation & Décision</h3>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-right">
              <div className="text-2xl font-bold text-gray-900">
                ...
                <span className="text-sm font-normal text-gray-500">€ HT/MWh</span>
              </div>
              <div className="text-sm text-gray-500">
                {Math.round(bestScored.annualCostP50).toLocaleString('fr-FR')} € HT/an
              </div>
            </div>
          </div>
      ...
      <div className="text-[10px] text-gray-400 pt-2 border-t border-gray-100">
        ...
        <ul className="space-y-0.5">
          <li>Simulation Monte Carlo (max 200 itérations) — valeur indicative</li>
          <li>Prix basé sur hypothèses de marché (voir paramètres)</li>
          <li>Scoring automatisé — validation humaine recommandée</li>
...
function AuditTrailView() {
  ...
  if (log.length === 0) {
    return <p className="text-sm text-gray-400 py-4">Aucune décision enregistrée.</p>;
  }
