...
describe('SolPanel keyboard navigation (A8)', () => {
  ...
  it('handles ArrowDown, ArrowUp, Home, End via KEY_NAV_NEXT table', () => {
    expect(src).toMatch(/KEY_NAV_NEXT/);
  ...
  it('Escape key blurs active element (exit panel)', () => {
  ...
  it('ArrowDown : clamp to len - 1', () => {
    expect(src).toMatch(/Math\.min\(idx \+ 1,\s*len - 1\)/);
  });
  ...
  it('Home : focus first item (returns 0)', () => {
    expect(src).toMatch(/Home:\s*\(\)\s*=>\s*0/);
  });

  it('End : focus last item (returns len - 1)', () => {
    expect(src).toMatch(/End:\s*\(_?idx,\s*len\)\s*=>\s*len - 1/);
  });
