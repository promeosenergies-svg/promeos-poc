...
function _getCoords(rect, position) {
  ...
  switch (position) {
    ...
    case 'left':
      return { top: cy, left: rect.left - OFFSET, transform: 'translate(-100%, -50%)' };
    case 'right':
      return { top: cy, left: rect.right + OFFSET, transform: 'translateY(-50%)' };
    default:
      return { top: rect.top - OFFSET, left: cx, transform: 'translate(-50%, -100%)' };
  }
}

export default function InfoTip({ content, position = 'top', size = 12, className = '' }) {
    ...
    <>
      ...
      {visible &&
        ...
        createPortal(
          ...
          >
            <div
              className="max-w-[280px] px-3 py-2 text-xs text-gray-700 leading-relaxed
