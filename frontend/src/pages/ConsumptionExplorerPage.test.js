...
vi.mock('../services/api', () => ({
  getConsumptionTunnel: vi.fn().mockResolvedValue({
    site_id: 1, energy_type: 'electricity', days: 90, readings_count: 500,
    envelope: { weekday: Array.from({ length: 24 }, (_, h) => ({ hour: h, p10: 5, p25: 8, p50: 12, p75: 16, p90: 20, count: 10 })), weekend: [] },
    outside_pct: 8.5, outside_count: 3, total_evaluated: 35, confidence: 'medium', confidence_score: 65,
  }),
  ...
  getTargetsProgression: vi.fn().mockResolvedValue({
    site_id: 1, year: 2026, yearly_target_kwh: 60000,
    ytd_actual_kwh: 9600, ytd_target_kwh: 10000, progress_pct: 96,
    forecast_year_kwh: 57600, forecast_vs_target_pct: -4, alert: 'on_track',
    months: Array.from({ length: 12 }, (_, i) => ({ month: i + 1, target_kwh: 5000, actual_kwh: i < 2 ? 4800 : null, delta_pct: i < 2 ? -4 : null })),
  }),
  getTOUSchedules: vi.fn().mockResolvedValue([]),
  getActiveTOUSchedule: vi.fn().mockResolvedValue({
    id: null, name: 'HC/HP Standard (defaut)', is_default: true,
    windows: [
    ...
    price_hp_eur_kwh: 0.18, price_hc_eur_kwh: 0.13,
  }),
  createTOUSchedule: vi.fn().mockResolvedValue({ id: 1 }),
  getHPHCRatio: vi.fn().mockResolvedValue({
    site_id: 1, hp_kwh: 3500, hc_kwh: 1500, total_kwh: 5000,
    hp_ratio: 0.7, hp_cost_eur: 630, hc_cost_eur: 195, total_cost_eur: 825,
    schedule_name: 'HC/HP Standard', confidence: 'medium',
  }),
  getGasSummary: vi.fn().mockResolvedValue({
    site_id: 1, energy_type: 'gas', days: 90, readings_count: 0,
    daily_kwh: [], total_kwh: 0, avg_daily_kwh: 0, summer_base_kwh: 0, confidence: 'low',
  }),
}));
