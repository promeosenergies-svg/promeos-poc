...
import { BadgeEuro, Plus, Trash2, Edit3 } from 'lucide-react';
import { Card, CardBody, Badge, Button, EmptyState, PageShell } from '../ui';
import { Table, Thead, Tbody, Th, Tr, Td } from '../ui';
import { SkeletonCard } from '../ui/Skeleton';
import { getPaymentRules, createPaymentRule, deletePaymentRule } from '../services/api';
...
export default function PaymentRulesPage() {
  ...
  return (
    <PageShell icon={BadgeEuro} title="Paiement & Refacturation" subtitle="Matrice facturé / payeur / centre de coûts">
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            {portfolioRules.length === 0 ? (
              <CardBody><p className="text-sm text-gray-500">Aucune règle portefeuille configurée</p></CardBody>
            ) : (
              <Table>
                <Thead>
                  <tr><Th>Portefeuille</Th><Th>EJ Facturée</Th><Th>EJ Payeur</Th><Th>Centre de coût</Th><Th></Th></tr>
                </Thead>
                <Tbody>
          ...
          {overrides.length > 0 && (
            <Card>
              ...
              <Table>
                <Thead>
                  <tr><Th>Niveau</Th><Th>Cible</Th><Th>EJ Facturée</Th><Th>EJ Payeur</Th><Th>Centre de coût</Th><Th></Th></tr>
                </Thead>
                <Tbody>
                  {overrides.map(r => (
                    <Tr key={r.id}>
                      <Td><Badge status={LEVEL_BADGE[r.level]?.status || 'info'}>{LEVEL_BADGE[r.level]?.label || r.level}</Badge></Td>
                      <Td className="font-medium">{r.site_id ? `Site #${r.site_id}` : r.contract_id ? `Contrat #${r.contract_id}` : '—'}</Td>
                      <Td>#{r.invoice_entity_id}</Td>
                      <Td>{r.payer_entity_id ? `#${r.payer_entity_id}` : '—'}</Td>
                      <Td>{r.cost_center || '—'}</Td>
                      <Td className="text-right">
