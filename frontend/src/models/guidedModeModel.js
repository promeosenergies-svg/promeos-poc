...
export const GUIDED_STEPS = [
  {
    id: 'assujettissement', order: 1, label: 'Assujettissement',
    description: 'Identifiez les réglementations qui s\'appliquent à vos sites.',
    cta: 'Voir les obligations', ctaTarget: { tab: 'obligations' },
  },
  {
    id: 'donnees', order: 2, label: 'Données', blocking: true,
    description: 'Vérifiez la qualité et complétude des données.',
    cta: 'Vérifier les données', ctaTarget: { tab: 'donnees' },
  },
  {
    id: 'deadlines', order: 3, label: 'Échéances',
    description: 'Visualisez les prochaines échéances réglementaires.',
    cta: 'Voir les échéances', ctaTarget: { tab: 'obligations' },
  },
  {
    id: 'plan', order: 4, label: 'Plan d\'action',
    description: 'Construisez votre plan de mise en conformité.',
    cta: 'Voir le plan', ctaTarget: { tab: 'execution' },
  },
  {
    id: 'cee_gouvernance', order: 5, label: 'CEE & gouvernance',
    description: 'Gérez les dossiers CEE et la gouvernance énergétique.',
    cta: 'Gérer les dossiers', ctaTarget: { path: '/compliance/pipeline' },
  },
  {
    id: 'preuves', order: 6, label: 'Preuves',
    description: 'Rassemblez les preuves de conformité.',
    cta: 'Joindre les preuves', ctaTarget: { tab: 'preuves' },
  },
  {
    id: 'mv', order: 7, label: 'M&V',
    description: 'Suivez la mesure et vérification des économies.',
    cta: 'Accéder au suivi', ctaTarget: { path: '/compliance/pipeline' },
  },
];
...
function evalStep(stepId, sitesData, summary, signals) {
  const {
    obligations = [], actionableFindings = [], proofFiles = {},
    workPackages, mvSummary,
  } = signals;
  ...
  switch (stepId) {
    ...
    case 'deadlines':
      ...
      return obligations.every(o =>
        o.echeance || o.statut === 'conforme' || o.statut === 'hors_perimetre'
    ...
    case 'cee_gouvernance':
      ...
      return workPackages.every(wp =>
        !wp.cee_step || wp.cee_step === 'submitted' || wp.cee_step === 'validated'
      ) ? 'complete' : 'in_progress';
...
export function computeDonneesMetrics(sitesData = [], dataQualityResults = [], signals = {}) {
  ...
  if (dataQualityResults.length > 0) {
    ...
    const filledCount = sitesData.filter(s =>
      s.findings?.length > 0 || s.conso_kwh_an || s.surface_m2
    ).length;
    completude_pct = Math.round((filledCount / sitesData.length) * 100);
  }

  // Confiance
  const avgConfidence = dataQualityResults.length > 0
    ? dataQualityResults.reduce((s, dq) => s + (dq.confidence_score || 0), 0) / dataQualityResults.length
    : (completude_pct >= 80 ? 75 : completude_pct >= 50 ? 50 : 25);

  let confiance_level, confiance_label;
  if (avgConfidence >= 70) { confiance_level = 'high'; confiance_label = 'Élevée'; }
  else if (avgConfidence >= 40) { confiance_level = 'medium'; confiance_label = 'Moyenne'; }
  else { confiance_level = 'low'; confiance_label = 'Faible'; }
  ...
  if (completude_pct < 80) {
    gaps.push({ id: 'patrimoine_incomplet', label: 'Données patrimoine incomplètes', ctaPath: '/patrimoine', ctaLabel: 'Compléter le patrimoine' });
  }
  if (sitesData.length > 0 && sitesData.every(s => !s.conso_kwh_an)) {
    gaps.push({ id: 'conso_manquante', label: 'Consommations manquantes', ctaPath: '/consommations', ctaLabel: 'Importer les consommations' });
  }
  if (billingMonths < couverture_factures_cible) {
    gaps.push({ id: 'factures_insuffisantes', label: 'Historique factures insuffisant', ctaPath: '/factures', ctaLabel: 'Compléter l\'historique' });
  }
