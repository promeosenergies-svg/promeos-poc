...
| **L7** Data Dictionary V4 | **Accepted** (2026-05-14) — voir [`docs/dev/L7_data_dictionary_v4.md`](../dev/L7_data_dictionary_v4.md) — manuel de référence unique pour tout dev Mois 2+ · compilation pure (pas d'arbitrage nouveau) · 70 termes glossaire (8 catégories) · 8 tables V4 + 20 indexes · 9 enums Python (LifecycleState/ClosureReason/PriorityBracket/EventType/RetentionCategory/Role/Domain/Kind/BlockerType) · 16 schemas Pydantic v1 référencés · 49 invariants quick-reference + 8 cardinaux Amine 🛡️ · 41 paires FR/EN · auto-éval 40/30 | §2 (axes orthogonaux), §3 (kinds), §4 (score), §5 (modulation R1-R6), §7 (libellés FR/EN) |
| **L8** Plan suppression legacy Mois 5 | **Accepted** (2026-05-14) — voir [`docs/dev/L8_plan_suppression_legacy.md`](../dev/L8_plan_suppression_legacy.md) — procédure opérationnelle step-by-step Mois 5 J+14 minimum (post-cutover Mois 4 + STOP GATE 8 critères ADR-026 §8.6) · 18 tables legacy à DROP (Alembic destructive) · ~1 667 LoC FE mortes (canonique L1 Annexe A) · ~9 models + 20 services + 51 endpoints backend · 173 rows data préservées Mois 4 J0 · 12 mois rétention RGPD CNIL · receipt sanitizé in Git (I9 cardinal Amine) · auto-éval 27/18 · ⚠️ **irréversible après exécution** | §7.1 (closure_reasons révisés v0.3 préservés post-suppression) |
| **L9** Mois 2 backend pilot manual | **Accepted** (2026-05-14) — voir [`docs/dev/L9_mois2_backend_pilotage.md`](../dev/L9_mois2_backend_pilotage.md) — **DERNIER LIVRABLE MOIS 1** · synthèse finale (9 livrables + 49 invariants + 46 arbitrages + 11 cardinaux non-rejouables) + sprint plan Mois 2 8 sprints M2-1→M2-8 + 50 source-guards activation progressive + pyramide tests 100/574 + DoD binaire 20 critères + transition pratiques · auto-éval 22/20 · 🎯 **Mois 1 docs only complet 10/10** | §11 historique versions (politique nouvelle évolution = avenant versionné Mois 2+) |
